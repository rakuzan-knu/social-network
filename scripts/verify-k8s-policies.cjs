...
const DEPLOYMENT_FILE = path.join(__dirname, '..', 'k8s', 'deployment.yaml');
...
function verifyK8sPolicies() {
  ...
  if (/image:.*:latest/.test(content)) {
    console.error(
      `❌ Policy Violation [no-latest-tag]: Found container image with ':latest' tag in k8s/deployment.yaml`,
    );
    errors++;
  } else {
  ...
  if (!content.includes('runAsNonRoot: true')) {
    console.error(
      `❌ Policy Violation [non-root]: Missing 'runAsNonRoot: true' in k8s/deployment.yaml`,
    );
    errors++;
  } else {
  ...
  if (!content.includes('readOnlyRootFilesystem: true')) {
    console.error(
      `❌ Policy Violation [readOnlyRootFS]: Missing 'readOnlyRootFilesystem: true' in k8s/deployment.yaml`,
    );
    errors++;
  } else {
  ...
  if (!content.includes('limits:') || !content.includes('requests:')) {
    console.error(
      `❌ Policy Violation [resource-limits]: Missing resource requests/limits in k8s/deployment.yaml`,
    );
    errors++;
  } else {
