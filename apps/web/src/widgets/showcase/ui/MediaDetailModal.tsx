...
const MediaRouterSync: React.FC<{
  isOpen: boolean;
  ...
  useEffect(() => {
    if (isOpen && activeItemTitle) {
      const currentParam = searchParams.get('mediaTitle');
      if (currentParam !== activeItemTitle) {
        setSearchParams(
          (prev) => {
            const next = new URLSearchParams(prev);
            next.set('mediaTitle', activeItemTitle);
            return next;
          },
          { replace: true },
        );
      }
    } else if (!isOpen && searchParams.has('mediaTitle')) {
      setSearchParams(
        (prev) => {
          const next = new URLSearchParams(prev);
  ...
  }, [isOpen, activeItemTitle, searchParams, setSearchParams]);
...
export const MediaDetailModal: React.FC = () => {
  ...
  if (!isOpen || !activeItem || !details) {
    return inRouter ? (
      <MediaRouterSync isOpen={isOpen} activeItemTitle={activeItem?.title} />
    ) : null;
  }
  ...
  return (
    <>
      {inRouter && <MediaRouterSync isOpen={isOpen} activeItemTitle={activeItem?.title} />}
      {createPortal(
        <div
                      ...
                      <>
                        ...
                        {isMuted && (
                          ...
                          >
                            ...
                            <span className="hidden sm:inline group-hover/unmute:inline">
                              Unmute
                            </span>
                          </button>
                        )}
                        ...
                        return (
                          ...
                          >
                            <img
                              ...
                              onError={(e) => {
                                (e.currentTarget as HTMLImageElement).src =
                                  details.miniPosterUrl ||
                                  getCategoryFallbackPoster(activeItem.type);
                              }}
                            />
                          </button>
                        );
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    {activeItem.type === ShowcaseMediaType.GAME && (
                      <div className="px-1 text-[11px] text-gray-400">
                        ...
                        <a
                          href={
                            details.details.developerClaimUrl || 'https://partner.steamgames.com/'
                          }
                          target="_blank"
                          rel="noopener noreferrer"
