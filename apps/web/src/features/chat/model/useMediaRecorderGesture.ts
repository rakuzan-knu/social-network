...
import { normalizePeaksTo100 } from '../lib/waveformUtils';
...
export function useMediaRecorderGesture({ onSend, onError }: UseMediaRecorderGestureProps) {
  ...
  // Compute final Telegram standard 45-bar waveform (0..100 normalized integer peaks)
  const computeFinalWaveform = useCallback(() => {
    return normalizePeaksTo100(waveformSamplesRef.current, 45);
  }, []);
