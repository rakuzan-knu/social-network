...
import { APPLICANT_PRIVACY_DATA } from './data/applicantPrivacyData';
...
export const ApplicantCandidatePrivacyPage: React.FC = () => {
  const { currentLanguage } = useLanguageStore();
  const isUkrainian = currentLanguage === 'Українська';
  const data = isUkrainian ? APPLICANT_PRIVACY_DATA.uk : APPLICANT_PRIVACY_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'the-information-we-collect',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white flex flex-col font-sans selection:bg-purple-500 selection:text-white relative">
      <SEOHead
        title="Applicant & Candidate Privacy Policy • Eternal Careers"
        description={data.hero.introLead}
        canonical="/terms/applicant-candidate-privacy-policy"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Applicant Privacy Policy', url: '/terms/applicant-candidate-privacy-policy' },
          ],
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 lg:px-12 pt-32 pb-24 w-full">
        ...
        <section className="mb-14 border-b border-purple-900/30 pb-10">
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-6 p-6 sm:p-8 rounded-3xl bg-[#110e20] border border-purple-500/20 text-neutral-200 text-base leading-relaxed space-y-4">
              <p className="font-semibold text-white">{data.hero.introLead}</p>
              <ul className="list-disc pl-6 space-y-2 text-purple-200">
                {data.hero.introBullets.map((b, i) => (
                  <li key={i}>{b}</li>
                ))}
              </ul>
              <p className="text-sm text-neutral-300 pt-2 border-t border-white/10">
                {data.hero.introDefinition}
              </p>
              <p className="text-sm text-neutral-300">
                ...
                >
                  <span>{data.hero.serviceLinkText}</span>
...
export default ApplicantCandidatePrivacyPage;
