...
import { DataPackageVisualGuide } from './ui/DataPackageVisualGuide';
import {
  Package,
  Download,
  FolderArchive,
...
export const YourDataPackagePage: React.FC = () => {
  const { currentLanguage } = useLanguageStore();
  const isUkrainian = currentLanguage === 'Українська';
  const data = isUkrainian ? DATA_PACKAGE_DATA.uk : DATA_PACKAGE_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'what-is-data-package',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white flex flex-col font-sans selection:bg-purple-500 selection:text-white relative">
      <SEOHead
        title="Your Eternal Data Package • Exporting Your Data"
        description={data.hero.description}
        canonical="/terms/your-eternal-data-package"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Your Data Package', url: '/terms/your-eternal-data-package' },
          ],
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 lg:px-12 pt-32 pb-24 w-full">
        ...
        {/* HERO SECTION                                                             */}
        {/* ========================================================================= */}
        <section className="mb-8 border-b border-purple-900/30 pb-10">
          <div className="flex flex-col gap-5">
            ...
            <p className="text-sm text-purple-300/80 font-medium">
              {isUkrainian
                ? 'Набуття чинності: 1 вересня 2026 р. • Останнє оновлення: 30 серпня 2026 р.'
                : 'Effective: September 1, 2026 • Last updated: August 30, 2026'}
            </p>

            <p className="mt-2 text-base sm:text-lg text-neutral-300 leading-relaxed max-w-4xl">
              {data.hero.description}
            </p>

            {/* What this article covers box */}
            <div className="mt-4 p-6 rounded-3xl bg-[#110e20] border border-purple-900/40 shadow-lg">
              <h4 className="text-sm font-bold uppercase tracking-wider text-purple-300 mb-3">
                {data.coversBox.title}
              </h4>
              <ul className="space-y-2 text-sm text-neutral-300">
                {data.coversBox.items.map((item) => (
                  <li key={item.id} className="space-y-1">
                    <button
                      type="button"
                      onClick={() => scrollToSection(item.id)}
                      className="text-purple-400 hover:text-purple-200 underline font-medium text-left"
                    >
                      • {item.label}
                    </button>
                        ...
                        {item.subItems.map((sub) => (
                          <li key={sub.id}>
                            <button
                              type="button"
                              onClick={() => scrollToSection(item.id)}
                              className="text-neutral-400 hover:text-purple-300 text-xs"
                            >
                              - {sub.label}
                            </button>
                          </li>
        ...
        {/* ========================================================================= */}
        {/* INTERACTIVE VISUAL GUIDE (Desktop & Mobile Walkthrough)                  */}
        {/* ========================================================================= */}
        <div className="print-hide">
          <DataPackageVisualGuide />
        </div>
                ...
                <div className="space-y-8">
                  {section.subsections.map((sub) => (
                    <div key={sub.id} className="space-y-4">
                      ...
                      {sub.steps && sub.steps.length > 0 && (
                        <div className="space-y-3 pt-2">
                          {sub.steps.map((step) => (
                            ...
                            >
                              <span className="w-7 h-7 rounded-xl bg-purple-600/30 border border-purple-500/40 text-purple-300 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                                {step.stepNumber}
                              </span>
                      ...
                      {/* Green Callout Tip */}
                      {sub.tip && (
                        <div className="p-4 rounded-2xl bg-emerald-950/25 border border-emerald-500/30 text-emerald-300 text-sm leading-relaxed flex items-start gap-3">
...
export default YourDataPackagePage;
