...
import { DEVELOPER_TERMS_DATA } from './data/developerTermsData';
...
export const DeveloperTermsPage: React.FC = () => {
  ...
  const data = isUkrainian ? DEVELOPER_TERMS_DATA.uk : DEVELOPER_TERMS_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'developer-overview',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-[#E0E0E6] font-sans antialiased selection:bg-purple-600 selection:text-white flex flex-col justify-between">
      <SEOHead
        title={data.hero.title || 'Developer Terms & API Guidelines'}
        description={data.hero.description}
        canonical="/terms/developer"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Developer Terms', url: '/terms/developer' },
          ],
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 pt-32 pb-24 w-full flex-1">
        ...
        <div className="max-w-4xl mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/60 border border-emerald-500/30 text-emerald-300 text-xs font-bold uppercase tracking-wider mb-6">
                  ...
                  return (
                    <a
                      ...
                      className={`group flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all ${
                        isActive
                          ? 'bg-emerald-600/20 text-emerald-200 border border-emerald-500/40 shadow-sm translate-x-1'
            ...
            {sections.map((section) => (
              ...
              >
                ...
                <div className="border-b border-purple-900/30 pb-4">
                  <div className="flex items-center gap-2.5 text-xs font-black tracking-widest text-emerald-400 uppercase mb-1">
