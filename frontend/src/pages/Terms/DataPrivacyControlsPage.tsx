...
import { InteractivePrivacyControlsMockup } from './ui/InteractivePrivacyControlsMockup';
import {
  Sliders,
  Eye,
  Shield,
...
import { DATA_PRIVACY_CONTROLS_DATA } from './data/dataPrivacyControlsData';
...
export const DataPrivacyControlsPage: React.FC = () => {
  const { currentLanguage } = useLanguageStore();
  const isUkrainian = currentLanguage === 'Українська';
  const data = isUkrainian ? DATA_PRIVACY_CONTROLS_DATA.uk : DATA_PRIVACY_CONTROLS_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'accessing-privacy-controls',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white flex flex-col font-sans selection:bg-purple-500 selection:text-white relative">
      <SEOHead
        title="Data Privacy Controls • Manage Your Data on Eternal"
        description={data.hero.description}
        canonical="/terms/data-privacy-controls"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Data Privacy Controls', url: '/terms/data-privacy-controls' },
          ],
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 lg:px-12 pt-32 pb-24 w-full">
        ...
        <section className="mb-8 border-b border-purple-900/30 pb-10">
          <div className="flex flex-col gap-4">
            ...
            <p className="mt-4 text-base sm:text-lg text-neutral-300 leading-relaxed max-w-4xl">
              {data.hero.description}
            </p>

            <div className="mt-4 p-4 rounded-2xl bg-purple-950/30 border border-purple-500/20 text-purple-300 text-sm font-medium">
              💡 {data.hero.locationNote}
            </div>
          </div>
        </section>

        {/* ========================================================================= */}
        {/* INTERACTIVE VISUAL GUIDE (Settings Tab Replica)                         */}
        {/* ========================================================================= */}
        <div className="print-hide">
          <InteractivePrivacyControlsMockup />
        </div>
...
export default DataPrivacyControlsPage;
