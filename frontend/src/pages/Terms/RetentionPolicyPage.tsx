...
import { RETENTION_POLICY_DATA } from './data/retentionPolicyData';
...
export const RetentionPolicyPage: React.FC = () => {
  ...
  const data = isUkrainian ? RETENTION_POLICY_DATA.uk : RETENTION_POLICY_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'self-service-deletion',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white flex flex-col font-sans selection:bg-purple-500 selection:text-white relative">
      <SEOHead
        title="Data Retention Policy • How Long We Keep Your Data"
        description={data.hero.description}
        canonical="/terms/retention-policy"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Data Retention Policy', url: '/terms/retention-policy' },
          ],
