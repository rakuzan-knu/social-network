...
import { COPYRIGHT_POLICY_DATA } from './data/copyrightPolicyData';
...
export const CopyrightPolicyPage: React.FC = () => {
  ...
  const data = isUkrainian ? COPYRIGHT_POLICY_DATA.uk : COPYRIGHT_POLICY_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'copyright-overview',
  );
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-[#E0E0E6] font-sans antialiased selection:bg-purple-600 selection:text-white flex flex-col justify-between">
      <SEOHead
        title={data.hero.title || 'Copyright & DMCA Policy'}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 pt-32 pb-24 w-full flex-1">
        ...
        <div className="max-w-4xl mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-purple-950/60 border border-purple-500/30 text-purple-300 text-xs font-bold uppercase tracking-wider mb-6">
            ...
            {sections.map((section) => (
              ...
              >
                ...
                <div className="border-b border-purple-900/30 pb-4">
                  <div className="flex items-center gap-2.5 text-xs font-black tracking-widest text-purple-400 uppercase mb-1">
                      ...
                      {sub.bullets && sub.bullets.length > 0 && (
                        <ul className="flex flex-col gap-2.5 pl-2">
                          {sub.bullets.map((b, bIdx) => (
                            ...
                            >
                              <span className="w-1.5 h-1.5 rounded-full bg-purple-400 mt-2 shrink-0 shadow-[0_0_8px_rgba(192,132,252,0.8)]" />
                              <span>{b}</span>
                            </li>
                          ))}
                        </ul>
