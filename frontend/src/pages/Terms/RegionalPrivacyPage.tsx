...
import { REGIONAL_PRIVACY_DATA } from './data/regionalPrivacyData';
...
export const RegionalPrivacyPage: React.FC = () => {
  const { currentLanguage } = useLanguageStore();
  const isUkrainian = currentLanguage === 'Українська';
  const data = isUkrainian ? REGIONAL_PRIVACY_DATA.uk : REGIONAL_PRIVACY_DATA.en;
  const sections = data.sections;
  const tToc = data.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(sections[0]?.id || 'eea-uk-gdpr');
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white flex flex-col font-sans selection:bg-purple-500 selection:text-white relative">
      <SEOHead
        title="Regional Privacy Rights • GDPR & CCPA Protections"
        description={data.hero.description}
        canonical="/terms/regional-privacy"
        structuredData={{
          breadcrumbs: [
            { name: 'Terms of Service', url: '/terms' },
            { name: 'Regional Privacy', url: '/terms/regional-privacy' },
          ],
...
export default RegionalPrivacyPage;
