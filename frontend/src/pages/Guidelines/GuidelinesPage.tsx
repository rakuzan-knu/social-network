...
import { getGuidelinesTranslation } from './data/guidelinesTranslations';
...
export const GuidelinesPage: React.FC = () => {
  ...
  const guidelinesData = getGuidelinesTranslation(currentLanguage);
  const sections = guidelinesData.sections;
  const tToc = guidelinesData.toc;

  const [activeSectionId, setActiveSectionId] = useState<string>(
    sections[0]?.id || 'welcome-and-values',
  );
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 2. Measure against the main guidelines article content
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-[#E0E0E6] font-sans antialiased selection:bg-purple-600 selection:text-white">
      <SEOHead
        title={guidelinesData.hero.title || 'Community Guidelines'}
        description={guidelinesData.hero.description}
        canonical="/guidelines"
        structuredData={{
          breadcrumbs: [{ name: 'Community Guidelines', url: '/guidelines' }],
      ...
      <section className="pt-32 pb-12 px-6 lg:px-12 max-w-7xl mx-auto border-b border-purple-900/20">
        <div className="max-w-3xl">
          ...
          <p className="text-xs sm:text-sm font-medium text-neutral-400 mb-6">
            {guidelinesData.hero.effectiveDate}
          </p>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          {/* Main Guidelines Article */}
          <main className="lg:col-span-8 flex flex-col gap-12 guidelines-content">
            {sections.map((section) => (
              <section
                key={section.id}
                id={section.id}
